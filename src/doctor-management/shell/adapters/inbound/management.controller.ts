...
import { PrismaManagementRepository } from "../outbound/prismaManagement.repository";
...
export class ManagementController {
  ...
  constructor() {
    ...
    // const prismaRepository = new PrismaManagementRepository();
  ...
  private initializeRoutes() {
    this.router.get(
      ...
      this.getUpcomingAppointments.bind(this)
    );

    this.router.patch(
      "/:appointmentId/complete",
      this.completeAppointment.bind(this)
    );
  ...
  private async getUpcomingAppointments(req: Request, res: Response) {
    try {
      const appointments =
        await this.managementService.getUpcomingAppointments();
  ...
  private async completeAppointment(req: Request, res: Response) {
    ...
    try {
      const updated = await this.managementService.markAppointmentAsCompleted(
        appointmentId
      );
  ...
  private async cancelAppointment(req: Request, res: Response) {
    const { appointmentId } = req.params;
    try {
      const updated = await this.managementService.cancelAppointment(
        appointmentId
      );
      return res.json(updated);
    } catch (error: any) {
